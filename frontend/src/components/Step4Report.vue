            ...
            >
              ...
              <div class="section-body" v-show="!collapsedSections.has(idx)">
                ...
                <div v-else-if="currentSectionIndex === idx + 1" class="loading-state">
                  ...
                  <span class="loading-text">Currently Generating{{ section.title }}...</span>
                </div>
              </div>
            </div>
          </div>
            ...
            >
              ...
              <div class="timeline-content">
                ...
                <div class="timeline-body" :class="{ 'collapsed': isLogCollapsed(log) }" @click="toggleLogExpand(log)">
                  ...
                  <!-- Section content generated (content generating complete, but whole section may not be done) -->
                  ...
                  <!-- Section Complete (SectionGeneratingComplete) -->
                  ...
                  <template v-if="log.action === 'llm_response'">
                    ...
                    <!-- When it's the final answer, show special prompt -->
...
const toggleRawResult = (timestamp, event) => {
  ...
  // Wait for DOM update, adjust scroll to keep button at same position
...
const parseInsightForge = (text) => {
  ...
  try {
    ...
    // extract statistics - match"RelatedPredictionfacts: Xitems"Format
    const factMatch = text.match(/RelatedPredictionfacts:\s*(\d+)/)
    const entityMatch = text.match(/involvesEntity:\s*(\d+)/)
    const relMatch = text.match(/Relationschain:\s*(\d+)/)
    ...
    // extract core entity - full extraction, contains summary and related facts count
    ...
    // extractRelationschain - Full extraction，Unlimited
    const relSection = text.match(/### 【Relationschain】\n([\s\S]*?)(?=\n###|$)/)
    if (relSection) {
...
const parsePanorama = (text) => {
  ...
  try {
    ...
    // extracthistory/Expired facts - Full extraction，Unlimited
    ...
    // extractinvolvesEntity - Full extraction，Unlimited
    const entitySection = text.match(/### 【involvesEntity】\n([\s\S]*?)(?=\n###|$)/)
    if (entitySection) {
...
const parseInterview = (text) => {
  ...
  try {
    // extract interview theme
    const topicMatch = text.match(/\*\*Interview theme:\*\*\s*(.+?)(?:\n|$)/)
    if (topicMatch) result.topic = topicMatch[1].trim()
    
    // extract interviewee count (e.g. "5 / 9 agent simulation agents")
    ...
    const parseIndividualReasons = (reasonText) => {
      ...
      for (const line of lines) {
        ...
        // e.g.: 1. **alumni_345（index=1）**：as Wuhan University alumnus...
        ...
        // e.g.: - select parent_601（index 0）：as parent grouprepresents...
        if (!headerMatch) {
          headerMatch = line.match(/^-\s*select\1(index\s*=?\s*\d+[)）])?[：:]\s*(.*)/)
          if (headerMatch) {
            name = headerMatch[1].trim()
            reasonStart = headerMatch[2]
          }
        ...
        // e.g.: - **Parent_601 (index 0)**: as parent group representativerepresents...
        if (!headerMatch) {
        ...
        } else if (currentName && line.trim() && !line.match(/^unselected|^in summary|^Finalselect/)) {
          // continuation of reason (exclude closing summary)
    ...
    interviewBlocks.forEach((block, index) => {
      ...
      if (qMatch) {
        ...
        if (questions.length > 0) {
          // if first question is preceded by "1.", needs special handlingHandle
      ...
      // extractanswer - split byTwitterandReddit
      ...
      if (answerMatch) {
        ...
        if (!twitterMatch && redditMatch) {
          ...
          // No per-platform markers (very old format), use whole as answer
    ...
    const summaryMatch = text.match(/### Interview Summary with Core Opinion\n([\s\S]*?)$/)
    if (summaryMatch) {
...
const parseQuickSearch = (text) => {
  ...
  try {
    ...
    // extractRelatedfacts - Full extraction，Unlimited
    ...
    return () => h('div', { class: 'insight-display' }, [
      ...
      h('div', { class: 'insight-tabs' }, [
        ...
        h('button', {
          ...
          h('span', { class: 'tab-label' }, `Relationschain (${props.result.relations.length})`)
        ]),
      ...
      h('div', { class: 'insight-content' }, [
        ...
        activeTab.value === 'facts' && props.result.facts.length > 0 && h('div', { class: 'facts-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'temporal memory associatedLatestKey facts'),
        ...
        activeTab.value === 'entities' && props.result.entities.length > 0 && h('div', { class: 'entities-panel' }, [
          ...
          }, expandedEntities.value ? `Collapse ▲` : `ExpandAll ${props.result.entities.length}  ▼`)
        ]),
        
        // Relations Tab
        activeTab.value === 'relations' && props.result.relations.length > 0 && h('div', { class: 'relations-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Relationschain'),
        ...
        activeTab.value === 'subqueries' && props.result.subQueries.length > 0 && h('div', { class: 'subqueries-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Drift query generating analysis sub-questions'),
        ...
        activeTab.value === 'entities' && props.result.entities.length === 0 && h('div', { class: 'empty-state' }, 'NoneCoreEntity'),
        activeTab.value === 'relations' && props.result.relations.length === 0 && h('div', { class: 'empty-state' }, 'NoneRelationschain')
      ])
    ])
  }
}
    ...
    return () => h('div', { class: 'panorama-display' }, [
      ...
      h('div', { class: 'panorama-tabs' }, [
        ...
        h('button', {
          ...
          h('span', { class: 'tab-label' }, `involvesEntity (${props.result.entities.length})`)
        ])
      ]),
      ...
      h('div', { class: 'panorama-content' }, [
        ...
        activeTab.value === 'active' && h('div', { class: 'facts-panel active-facts' }, [
          ...
          ) : h('div', { class: 'empty-state' }, 'NoneCurrent active memory'),
        ...
        activeTab.value === 'historical' && h('div', { class: 'facts-panel historical-facts' }, [
          ...
          ) : h('div', { class: 'empty-state' }, 'Nonehistorical memory'),
        ...
        activeTab.value === 'entities' && h('div', { class: 'entities-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'involvesEntity'),
            h('span', { class: 'panel-count' }, `total ${props.result.entities.length} `)
          ]),
          ...
          ) : h('div', { class: 'empty-state' }, 'NoneinvolvesEntity'),
...
const InterviewDisplay = {
  ...
  setup(props) {
    ...
    const isPlaceholderText = (text) => {
      if (!text) return true
      const t = text.trim()
      return t === '（No response received on this platform）' || t === '(No response received on this platform)' || t === '[NoneReplies]'
    }
    ...
    const splitAnswerByQuestions = (answerText, questionCount) => {
      ...
      // 1. "QuestionX:" (text format, new backend format)
      ...
      // extract each section by number
      const parts = []
    ...
    // Check if question has dual-platform response (filter placeholder text)
    ...
    return () => h('div', { class: 'interview-display' }, [
      ...
      props.result.interviews.length > 0 && h('div', { class: 'interview-detail' }, [
        ...
        // Q&A Conversation Thread - one question, one answerStyle
              ...
              answerText && h('div', { class: ['qa-answer', { 'answer-placeholder': isPlaceholder }] }, [
                ...
                h('div', { class: 'qa-content' }, [
                  h('div', { class: 'qa-answer-header' }, [
                    ...
                    // Dual-platform toggle button (show only when real dual-platform response exists)
                  ...
                  // Expand/Collapse Button（placeholder text hidden）
...
const renderMarkdown = (content) => {
  ...
  // Cleanup all whitespace between list items
  html = html.replace(/<\/li>\s+<li/g, '</li><li')
  // Cleanup whitespace after list opening tag
  html = html.replace(/<ul class="md-ul">\s+/g, '<ul class="md-ul">')
  html = html.replace(/<ol class="md-ol">\s+/g, '<ol class="md-ol">')
  // Cleanup whitespace before list closing tag
  ...
  // Handle line breaks - empty lines become paragraph breaks, single breaks become <br>
  html = html.replace(/\n\n/g, '</p><p class="md-p">')
  html = html.replace(/\n/g, '<br>')
  ...
  // Cleanuparound block elements <br> Tag
  html = html.replace(/<br>\s*(<ul|<ol|<blockquote)/g, '$1')
  html = html.replace(/(<\/ul>|<\/ol>|<\/blockquote>)\s*<br>/g, '$1')
  // Cleanup <p><br> immediately followed by block elements (caused by extra blank lines)
  html = html.replace(/<p class="md-p">(<br>\s*)+(<ul|<ol|<blockquote|<pre|<hr)/g, '$2')
  // Cleanup consecutive <br> tags
  html = html.replace(/(<br>\s*){2,}/g, '<br>')
  // Cleanupbefore paragraph tag immediately following block element <br>
  html = html.replace(/(<\/ol>|<\/ul>|<\/blockquote>)<br>(<p|<div)/g, '$1$2')

  // fixnon-consecutive ordered list numbering：when single item <ol> when separated by paragraph content，maintainincrementing numbering
    ...
    if (res.success && res.data) {
      ...
      if (newLogs.length > 0) {
        newLogs.forEach(log => {
          ...
          // section_complete - SectionGeneratingComplete
        ...
        nextTick(() => {
          if (rightPanel.value) {
            // if task completed, scroll to top; otherwise scroll to footer to followLatestLogs
            if (isComplete.value) {
...
// extractFinalanswerContent - from LLM response extractSectionContent
const extractFinalContent = (response) => {
  if (!response) return null
  
  // tryextract <final_answer> TagwithinContent
  ...
  // try to find content after "Final Answer:" (supports multipleFormat）
  ...
  // try to find content after "Final answer:"Content
  const chineseFinalMatch = response.match(/Finalanswer[:：]\s*\n*([\s\S]*)$/i)
  if (chineseFinalMatch) {
    return chineseFinalMatch[1].trim()
  }
  
  // if starting with ## or # or > at the start, may be direct markdown Content
  const trimmedResponse = response.trim()
  if (trimmedResponse.match(/^[#>]/)) {
    return trimmedResponse
  }
  
  // if content is long and contains markdown, try removing thinking process firstBack
  if (response.length > 300 && (response.includes('**') || response.includes('>'))) {
    // Remove Thought: leading thinking process
...
.section-number {
  ...
  font-size: 16px;
  color: #9CA3AF; /* dark gray，not followingStatusChanges */
  font-weight: 500;
}
...
/* Console Logs - with Step3Simulation.vue keep consistent */
.console-logs {
  background: #000;
  color: #DDD;
  padding: 16px;
