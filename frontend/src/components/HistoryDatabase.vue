<template>
  ...
  >
      ...
      >
        ...
        <div class="card-files-wrapper">
          ...
          <div class="files-list" v-if="project.files && project.files.length > 0">
            ...
            <!-- if there are more files, show prompt -->
          ...
          <div class="files-empty" v-else>
            ...
            <span class="empty-file-text">NoneFile</span>
          </div>
        </div>

        <!-- card title (using first 20 chars of simulation requirements) -->
        <h3 class="card-title">{{ getSimulationTitle(project.simulation_requirement) }}</h3>

        <!-- card description (full simulation requirements display) -->
        ...
        <!-- footer decorative line (expands on hover) -->
    ...
    <!-- history playback details popup -->
    <Teleport to="body">
        ...
        <div v-if="selectedProject" class="modal-overlay" @click.self="closeModal">
          <div class="modal-content">
            ...
            <div class="modal-body">
              ...
              <!-- FileList -->
              <div class="modal-section">
                <div class="modal-label">associatedFile</div>
                ...
                <div class="modal-empty" v-else>NoneassociatedFile</div>
              </div>
            </div>

            <!-- simulation playback split line -->
            ...
            <div class="modal-actions">
              ...
              >
                ...
                <span class="btn-icon">◆</span>
                <span class="btn-text">AnalysisReport</span>
              </button>
            </div>
            <!-- non-replayablePrompt -->
            <div class="modal-playback-hint">
              <span class="hint-text">Step3 'Run Simulation' and Step5 'Deep Interaction' require an active run and don't support history playback</span>
            </div>
          </div>
        </div>
      </Transition>
...
const selectedProject = ref(null)  // currently selected project (for popup)
let observer = null
let isAnimating = false  // animation lock to prevent flickering
let expandDebounceTimer = null  // debounce timer
let pendingState = null  // record target state pending execution
...
// calculate container height style based on activity
const containerStyle = computed(() => {
  ...
  // calculate required height: rows * card height + (rows-1) * gap + small footer gap
...
const getCardStyle = (index) => {
  ...
  if (isExpanded.value) {
    ...
    // calculate card count per row, ensure centered
...
// Formatformat simulation_id display (first 6 chars）
...
// truncate file name (preserve extension)
...
// NavigationtoGraph Buildpage（Project）
const goToProject = () => {
...
// NavigationtoAnalysisReport Page（Report）
const goToReport = () => {
...
const loadHistory = async () => {
  try {
    ...
    console.error('loadingPast ProjectsFailed:', error)
    projects.value = []
  } finally {
    loading.value = false
  }
  ...
  observer = new IntersectionObserver(
    (entries) => {
      entries.forEach((entry) => {
        ...
        // update target state pending execution (always record latest target regardless of animation)
        ...
        // if currently animating, only record state and wait for animation to finishHandle
        if (isAnimating) return
        
        // if target state matches current state, no need toHandle
        ...
        // shorter delay (50ms) for expand, longer (200ms) for collapse to increase stability
        ...
        expandDebounceTimer = setTimeout(() => {
          ...
          // animationCompleteunlock after and check for pending itemsStatusChanges
          setTimeout(() => {
            isAnimating = false
            
            // after animation ends, check for new pending state
            if (pendingState !== null && pendingState !== isExpanded.value) {
              // delay a short time before executing, to avoid being too fasttoggle
              expandDebounceTimer = setTimeout(() => {
                if (pendingState !== null && pendingState !== isExpanded.value) {
                  isAnimating = true
                  isExpanded.value = pendingState
    ...
    {
      // Usingmultiple thresholds for smoother detection
      threshold: [0.4, 0.6, 0.8],
      // adjust rootMargin, viewport footer contracts upward, requires more scroll to trigger expand
...
onMounted(async () => {
  ...
  // Waiting DOM Renderinitialize observer after
  setTimeout(() => {
    initObserver()
  }, 100)
})

// if using keep-alive, when component activatesReloadData
onActivated(() => {
  loadHistory()
})
...
/* simple format display when no projects */
...
/* UsingCSSbackground pattern creates a fixed-spacing square grid */
.grid-pattern {
  position: absolute;
  top: 0;
  left: 0;
  ...
  /* fromposition from top-left, height changes only expand at footer, don't affect existing grid positions */
...
.cards-container {
  ...
  /* min-height calculated by JS, adaptive based on card count */
}
...
.status-icon:nth-child(3).available { color: #10B981; } /* AnalysisReport - green */
...
/* ===== history playback details popupStyle ===== */
.modal-overlay {
  position: fixed;
  top: 0;
  left: 0;
