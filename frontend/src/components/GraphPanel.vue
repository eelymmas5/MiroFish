  ...
  <div class="graph-panel">
    <div class="panel-header">
      ...
      <div class="header-tools">
        ...
        <button class="tool-btn" @click="$emit('toggle-maximize')" title="Maximumtoggle fullscreen">
    ...
    <div class="graph-container" ref="graphContainer">
      ...
      <div v-if="graphData" class="graph-view">
        ...
        <div v-if="showSimulationFinishedHint" class="graph-building-hint finished-hint">
          ...
          <span class="hint-text">A few items still processing, consider manually refreshing the graph shortly</span>
          <button class="hint-close-btn" @click="dismissFinishedHint" title="ClosePrompt">
...
// close simulation-finished prompt
const dismissFinishedHint = () => {
  showSimulationFinishedHint.value = false
}

// listen for isSimulating changes, detect simulation end
watch(() => props.isSimulating, (newValue, oldValue) => {
  if (wasSimulating.value && !newValue) {
    // changed from simulation to non-simulation state, show end prompt
...
const renderGraph = () => {
  ...
  // handle edge data, calculate edge count and index between same node pairs
  const edgePairCount = {}
  ...
  // record which edge item is currently being handled for each node pair
  const edgePairIndex = {}
  const processedSelfLoopNodes = new Set() // already-handled self-loop nodes
  ...
  tempEdges.forEach(e => {
    ...
    if (isSelfLoop) {
      ...
      edges.push({
        ...
        rawData: {
          ...
          selfLoopEdges: allSelfLoops // store all self-loop edgesDetailedInfo
        }
      })
      return
    }
    ...
    // determine if edge direction matches standard (source UUID < target UUID)
    ...
    if (totalCount > 1) {
      ...
      // if edge direction is opposite to standard format, flip curvature
      // ensures all edges distribute in the same reference frame, won't overlap due to direction differences
      if (isReversed) {
        curvature = -curvature
      }
    }
  ...
  // Simulation - adjust node spacing based on edge count
  const simulation = d3.forceSimulation(nodes)
    .force('link', d3.forceLink(edges).id(d => d.id).distance(d => {
      // adjust distance based on edge count between this node pair
      // base distance 150, increases per extra edge 40
    ...
    // add centripetal force to gather isolated node clusters toward center
  ...
  // Links - using path to support curves
  ...
  const getLinkPath = (d) => {
    ...
    // detectSelf-loop
    if (d.isSelfLoop) {
      // Self-loop：draw an arc starting from node thenBack
      const loopRadius = 30
      // fromNodesstart from right, loop back
      const x1 = sx + 8  // start point offset
      const y1 = sy - 4
      const x2 = sx + 8  // endpoint offset
      const y2 = sy + 4
      // Usingarc drawingSelf-loop（sweep-flag=1 clockwise）
    ...
    // calculatecurve control point - based on edge count and distanceActivityadjust
    const dx = tx - sx, dy = ty - sy
    const dist = Math.sqrt(dx * dx + dy * dy)
    // perpendicular offset to connection direction, calculated by distance ratio to ensure curve is clearly visible
    // more edges means，the larger the offset ratio to distance
    const pairTotal = d.pairTotal || 1
    const offsetRatio = 0.25 + pairTotal * 0.05 // base25%，increases per extra edge5%
  ...
  // calculatecurve point (for label positioning）
  const getLinkMidpoint = (d) => {
    ...
    // detectSelf-loop
    if (d.isSelfLoop) {
      // Self-loopTagposition: right of nodes
      return { x: sx + 70, y: sy }
    }
    ...
    // point on quadratic bezier curve at t=0.5
    const dx = tx - sx, dy = ty - sy
    const dist = Math.sqrt(dx * dx + dy * dy)
    const pairTotal = d.pairTotal || 1
    ...
    // quadratic bezier curveformula B(t) = (1-t)²P0 + 2(1-t)tP1 + t²P2, t=0.5
  ...
  const link = linkGroup.selectAll('path')
    ...
    .on('click', (event, d) => {
      ...
      // Resetpreviously selected edgeStyle
  ...
  // Savequotefor external show/hide control
  ...
  const node = nodeGroup.selectAll('circle')
    ...
    .call(d3.drag()
      ...
      .on('drag', (event, d) => {
        // detectwhether actual drag started (movement exceeded threshold）
        const dx = event.x - d._dragStartX
        const dy = event.y - d._dragStartY
        const distance = Math.sqrt(dx * dx + dy * dy)
        
        if (!d._isDragging && distance > 3) {
          // first detect actual drag，before restarting simulation
          d._isDragging = true
      ...
      .on('end', (event, d) => {
        // only let simulation gradually stabilize after actual draggingStop
        if (d._isDragging) {
  ...
  simulation.on('tick', () => {
    ...
    // Updateedge label position (no rotation，horizontal display is clearer）
    linkLabels.each(function(d) {
      const mid = getLinkMidpoint(d)
      d3.select(this)
        .attr('x', mid.x)
        .attr('y', mid.y)
        .attr('transform', '') // Removerotate，maintainhorizontal
    })
    
    // Updateedge label background
  ...
  // click on whitespaceClose details panel
  svg.on('click', () => {
    selectedItem.value = null
    node.attr('stroke', '#fff').attr('stroke-width', 2.5)
    linkGroup.selectAll('path').attr('stroke', '#C0C0C0').attr('stroke-width', 1.5)
...
// listen for edge label show toggle
