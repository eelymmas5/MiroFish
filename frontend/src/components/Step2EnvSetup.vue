    ...
    <div class="scroll-container">
      ...
      <div class="step-card" :class="{ 'active': phase === 0, 'completed': phase > 0 }">
        ...
        <div class="card-content">
          ...
          <p class="description">
            Create a new simulation instance and fetch world parameter template
          </p>
      ...
      <div class="step-card" :class="{ 'active': phase === 1, 'completed': phase > 1 }">
        ...
        <div class="card-content">
          ...
          <p class="description">
            Using context, auto-calls tools from Knowledge Graph to organize entities/relations, initializes simulation agents, assigns unique behaviors and memories from real-world seeds
          </p>
          ...
          <div v-if="profiles.length > 0" class="stats-grid">
            ...
            <div class="stat-card">
              ...
              <span class="stat-label">Current seed-related topic count</span>
            </div>
          </div>
      ...
      <div class="step-card" :class="{ 'active': phase === 2, 'completed': phase > 2 }">
        ...
        <div class="card-content">
          <p class="api-note">POST /api/simulation/prepare</p>
          <p class="description">
            LLM intelligently sets world time speed, recommended algorithms, per-agent active slots, post frequency, event triggers and more based on Simulation Requirements and real-world seeds
          </p>
          ...
          <div v-if="simulationConfig" class="config-detail-panel">
            ...
            <div class="config-block">
              <div class="config-grid">
                ...
                <div class="config-item">
                  <span class="config-item-label">Active hours/day</span>
            ...
            <div class="config-block">
              <div class="config-block-header">
                <span class="config-block-title">Recommended Algorithm Config</span>
              </div>
            ...
            <!-- LLM Config inference -->
            <div v-if="simulationConfig.generation_reasoning" class="config-block">
              <div class="config-block-header">
                <span class="config-block-title">LLM Config inference</span>
              </div>
      ...
      <div class="step-card" :class="{ 'active': phase === 3, 'completed': phase > 3 }">
        ...
        <div class="card-content">
          <p class="api-note">POST /api/simulation/prepare</p>
          <p class="description">
            based on narrative direction, auto-generate initial activation events with trending topics to guide simulated worldinitialStatus
          </p>
      ...
      <div class="step-card" :class="{ 'active': phase === 4 }">
        ...
        <div class="card-content">
          ...
          <p class="description">Simulation EnvironmentalreadyPreparation complete，can start running simulation</p>
          
          <!-- Simulation RoundsConfig - only shown when config generation is complete and round count is calculated -->
              ...
              <div v-else class="rounds-content auto" key="auto">
                <div class="auto-info-card">
                  ...
                  <div class="auto-content">
                    ...
                    <div class="auto-desc">
                      <p class="highlight-tip" @click="useCustomRounds = true">for first run，strongly recommend switching to‘Custompattern’Reduce simulation rounds for quick preview and lower error risk ➝</p>
                    </div>
                  </div>
                </div>
              </div>
      ...
      <div v-if="selectedProfile" class="profile-modal-overlay" @click.self="selectedProfile = null">
        ...
        <div class="modal-body">
          ...
          <div class="modal-info-grid">
            <div class="info-item">
              <span class="info-label">Eventapparent age</span>
              <span class="info-value">{{ selectedProfile.age || '-' }}  years old</span>
            </div>
            <div class="info-item">
              <span class="info-label">Eventapparent gender</span>
            ...
            <div class="info-item">
              <span class="info-label">EventapparentMBTI</span>
          ...
          <div class="modal-section" v-if="selectedProfile.persona">
            ...
            <div class="persona-dimensions">
              <div class="dimension-card">
                <span class="dim-title">Eventpanoramic experience</span>
                <span class="dim-desc">complete behavioral trajectory for this event</span>
              </div>
              <div class="dimension-card">
              ...
              <div class="dimension-card">
                <span class="dim-title">Social relations network</span>
                <span class="dim-desc">connected to interaction graph</span>
              </div>
            </div>
...
const props = defineProps({
  simulationId: String,  // frompassed from parent component
  projectData: Object,
  graphData: Object,
  systemLogs: Array
})
...
// State
const phase = ref(0) // 0: Initialize, 1: GeneratingPersona, 2: GeneratingConfig, 3: Complete
...
// Logsdeduplicate：recordlast outputkeyInfo
let lastLoggedMessage = ''
let lastLoggedProfileCount = 0
let lastLoggedConfigStage = ''

// Simulation RoundsConfig
const useCustomRounds = ref(false) // DefaultUsingauto-configure round count
const customMaxRounds = ref(40)   // DefaultRecommended40 round

// Watch stage to update phase
watch(currentStage, (newStage) => {
  if (newStage === 'GeneratingAgentPersona' || newStage === 'generating_profiles') {
    phase.value = 1
  } else if (newStage === 'GeneratingSimulation Config' || newStage === 'generating_config') {
    phase.value = 2
    // enter config-generating phase, start polling config
    if (!configTimer) {
...
// fromConfigcalculateauto-generated round count (not using hardcoded default value）
const autoGeneratedRounds = computed(() => {
  ...
  if (!totalHours || !minutesPerRound) {
    return null // ConfigDatawhen incompleteBack null
  }
  const calculatedRounds = Math.floor((totalHours * 60) / minutesPerRound)
  // ensureMaximum roundcount not less than 40 (recommended value），avoid sliding items rangeException
...
// get corresponding by agent_idusername
...
// calculatetotal topics associated with all personas
...
// HandleRun Simulationbutton click
const handleStartSimulation = () => {
  ...
  if (useCustomRounds.value) {
    ...
    addLog(`Run Simulation，Custom rounds: ${customMaxRounds.value}  round`)
  } else {
    // user chose to maintain auto-generated rounds, don't pass max_rounds parameter
    addLog(`Run Simulation，Usingauto-configure round count: ${autoGeneratedRounds.value}  round`)
  }
...
const startPrepareSimulation = async () => {
  ...
  // mark first stepComplete，startRound second step
  phase.value = 1
  addLog(`simulationinstanceCreated: ${props.simulationId}`)
  addLog('Currently prepareSimulation Environment...')
  ...
  try {
    ...
    if (res.success && res.data) {
      if (res.data.already_prepared) {
        addLog('detected already-completed preparation, using directly')
        await loadPreparedData()
        return
      }
      
      taskId.value = res.data.task_id
      addLog(`preparetaskalreadyStart`)
      addLog(`  └─ Task ID: ${res.data.task_id}`)
      
      // immediately set expected total agents (from prepare API return value)
      if (res.data.expected_entities_count) {
        expectedTotal.value = res.data.expected_entities_count
        addLog(`fromZepgraph readingto ${res.data.expected_entities_count} Entity`)
      ...
      startPolling()
      // start real-time getting profiles
      startProfilesPolling()
    } else {
      addLog(`prepareFailed: ${res.error || 'UnknownError'}`)
      emit('update-status', 'error')
    }
  } catch (err) {
    addLog(`prepareException: ${err.message}`)
    emit('update-status', 'error')
  }
}
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      if (data.status === 'completed' || data.status === 'ready' || data.already_prepared) {
        addLog('✓ preparationCompleted')
        stopPolling()
        stopProfilesPolling()
        await loadPreparedData()
      } else if (data.status === 'failed') {
        addLog(`✗ prepareFailed: ${data.error || 'UnknownError'}`)
        stopPolling()
        stopProfilesPolling()
      }
    }
    ...
    if (res.success && res.data) {
      ...
      if (currentCount > 0 && currentCount !== lastLoggedProfileCount) {
        ...
        if (expectedTotal.value && currentCount >= expectedTotal.value) {
          addLog(`✓ All ${currentCount} AgentPersonaGeneratingComplete`)
        }
      }
    }
  } catch (err) {
...
const fetchConfigRealtime = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // output config generating phase logs (avoid duplicates)
      if (data.generation_stage && data.generation_stage !== lastLoggedConfigStage) {
        lastLoggedConfigStage = data.generation_stage
        if (data.generation_stage === 'generating_profiles') {
          addLog('Currently GeneratingAgentPersonaConfig...')
        } else if (data.generation_stage === 'generating_config') {
          addLog('Currently callLLMGeneratingSimulation ConfigParameter...')
        }
      }
      
      // if config is already generating
      if (data.config_generated && data.config) {
        simulationConfig.value = data.config
        addLog('✓ Simulation ConfigGeneratingComplete')
        ...
        if (data.summary) {
          ...
          addLog(`  ├─ initialPosts: ${data.summary.initial_posts_count}items`)
        ...
        // showTimeConfigDetails
        if (data.config.time_config) {
          const tc = data.config.time_config
          addLog(`TimeConfig: per round${tc.minutes_per_round}minutes, total${Math.floor((tc.total_simulation_hours * 60) / tc.minutes_per_round)} round`)
        }
        ...
        stopConfigPolling()
        phase.value = 4
        addLog('✓ Environment SetupComplete，canRun Simulation')
...
const loadPreparedData = async () => {
  ...
  addLog(`alreadyloading ${profiles.value.length} AgentPersona`)

  // GetConfig（Usingreal-time interface）
  try {
    const res = await getSimulationConfigRealtime(props.simulationId)
    if (res.success && res.data) {
      if (res.data.config_generated && res.data.config) {
        simulationConfig.value = res.data.config
        addLog('✓ Simulation ConfigloadingSuccess')
        
        // showDetailedConfigSummary
        if (res.data.summary) {
          addLog(`  ├─ AgentCount: ${res.data.summary.total_agents}`)
          addLog(`  ├─ Simulation duration: ${res.data.summary.simulation_hours}hours`)
          addLog(`  └─ initialPosts: ${res.data.summary.initial_posts_count}items`)
        }
        ...
        // Confignot yet generated, startPolling
        addLog('config generating, start polling and waiting...')
        startConfigPolling()
      }
    }
  } catch (err) {
    addLog(`loadingConfigFailed: ${err.message}`)
    emit('update-status', 'error')
  }
}
