    ...
    <div class="scroll-container">
      ...
      <div class="step-card" :class="{ 'active': currentPhase === 0, 'completed': currentPhase > 0 }">
        ...
        <div class="card-content">
          ...
          <p class="description">
            LLMAnalysisdocumentContentwithSimulation Requirements，extractextract real-world seeds，auto-generate appropriate ontology structure
          </p>

          <!-- Loading / Progress -->
          <div v-if="currentPhase === 0 && ontologyProgress" class="progress-section">
            <div class="spinner-sm"></div>
            <span>{{ ontologyProgress.message || 'Currently Analysisdocument...' }}</span>
          </div>
      ...
      <div class="step-card" :class="{ 'active': currentPhase === 1, 'completed': currentPhase > 1 }">
        ...
        <div class="card-content">
          ...
          <p class="description">
            based on generated ontology, auto-chunks documents and calls Zep to build Knowledge Graph, extracting entities and relations with temporal memory and communitySummary
          </p>
      ...
      <div class="step-card" :class="{ 'active': currentPhase === 2, 'completed': currentPhase >= 2 }">
        ...
        <div class="card-content">
          ...
          <p class="description">Graph BuildCompleted，please proceed to Next Step for simulationEnvironment Setup</p>
          <button 
            class="action-btn" 
          ...
          >
            ...
            {{ creatingSimulation ? 'create...' : 'Proceed to Environment Setup ➝' }}
          </button>
        </div>
      </div>
    </div>
...
const handleEnterEnvSetup = async () => {
  if (!props.projectData?.project_id || !props.projectData?.graph_id) {
    console.error('MissingprojectorgraphInfo')
    return
  }
  ...
  try {
    ...
    if (res.success && res.data?.simulation_id) {
      ...
      alert('createSimulation Failed: ' + (res.error || 'UnknownError'))
    }
  } catch (err) {
    console.error('Create SimulationException:', err)
    alert('Create SimulationException: ' + err.message)
  } finally {
