    ...
    <div class="control-bar">
      ...
      <div class="action-controls">
        ...
        >
          ...
          {{ isGeneratingReport ? 'Start...' : 'Start Generating Result Report' }} 
          ...
          >
            ...
            <div class="timeline-card">
              ...
              <div class="card-body">
                ...
                <!-- CREATE_COMMENT: publishComments -->
...
const props = defineProps({
  simulationId: String,
  maxRounds: Number, // fromStep2incoming max rounds
  minutesPerRound: {
    type: Number,
...
// Computed
// show actions in time order (latest at bottom/footer)
...
// format simulation elapsed time (based on round count and minutes per round)
...
const resetAllState = () => {
  ...
  stopPolling()  // Stoppreviously existingPolling
}
...
const doStartSimulation = async () => {
  ...
  // first reset all states to avoid being affected by the previous simulation
  resetAllState()
  
  isStarting.value = true
  startError.value = null
  addLog('Currently StartDual-platform parallel simulation...')
  ...
  try {
    const params = {
      ...
      enable_graph_memory_update: true  // enable activity graphUpdate
    }
    
    if (props.maxRounds) {
      params.max_rounds = props.maxRounds
      addLog(`SetMaximumSimulation Rounds: ${props.maxRounds}`)
    }
    
    addLog('alreadyenable activity graph update mode')
    
    const res = await startSimulation(params)
    
    if (res.success && res.data) {
      if (res.data.force_restarted) {
        addLog('✓ cleaned up old simulation logs, re-running simulation')
      }
      addLog('✓ simulation engineStartSuccess')
      ...
      addLog(`✗ StartFailed: ${res.error || 'UnknownError'}`)
      emit('update-status', 'error')
    }
  } catch (err) {
    startError.value = err.message
    addLog(`✗ StartException: ${err.message}`)
    emit('update-status', 'error')
  } finally {
    isStarting.value = false
  }
...
const handleStopSimulation = async () => {
  ...
  addLog('Currently Stop Simulation...')
  ...
  try {
    ...
    if (res.success) {
      ...
      addLog(`StopFailed: ${res.error || 'UnknownError'}`)
    }
  } catch (err) {
...
// track per-platform last round, for detecting changes and outputting logs
...
const fetchRunStatus = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // detect round changes for each platform and outputLogs
      ...
      // detectwhether simulation isCompleted（via runner_status orplatformCompleteStatusdetermine）
      const isCompleted = data.runner_status === 'completed' || data.runner_status === 'stopped'
      
      // extraCheck：ifbackend hasn't had time toUpdate runner_status，but platform has alreadyReportComplete
      // viadetect twitter_completed and reddit_completed Statusdetermine
      const platformsCompleted = checkPlatformsCompleted(data)
      
      if (isCompleted || platformsCompleted) {
        if (platformsCompleted && !isCompleted) {
          addLog('✓ detecttoall platform simulations have ended')
        }
...
// Checkwhether all enabled platformsCompleted
const checkPlatformsCompleted = (data) => {
  // if no platformsData，Back false
  if (!data) return false
  
  // Checkper-platformCompleteStatus
  const twitterCompleted = data.twitter_completed === true
  const redditCompleted = data.reddit_completed === true
  
  // if at least one platform is complete, check if all enabled platforms are complete
  // via actions_count determinewhether platform is enabled (if count > 0 or running was previously true）
  ...
  // Checkwhether all enabled platforms haveCompleted
...
const fetchRunStatusDetail = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      // Using all_actions Getcomplete actionsList
      ...
      // new actions will appear atFooterAppend
    }
  } catch (err) {
...
const handleNextStep = async () => {
  ...
  if (isGeneratingReport.value) {
    addLog('Report GenerationRequestalreadySend，please wait...')
    return
  }
  
  isGeneratingReport.value = true
  addLog('Currently StartReport Generation...')
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      addLog(`✓ Report GenerationtaskalreadyStart: ${reportId}`)
      
      // Jump toReport Page
      router.push({ name: 'Report', params: { reportId } })
    } else {
      addLog(`✗ StartReport GenerationFailed: ${res.error || 'UnknownError'}`)
      isGeneratingReport.value = false
    }
  } catch (err) {
    addLog(`✗ StartReport GenerationException: ${err.message}`)
    isGeneratingReport.value = false
  }
}
...
onMounted(() => {
  addLog('Step3 simulation runInitialize')
  if (props.simulationId) {
    doStartSimulation()
  }
})
