            ...
            <div class="phase-detail">
              <div class="detail-section">
                ...
                <div class="detail-content">
                  After uploading documents, the LLM analyzes content and auto-generates an ontology structure (Entity Types + Relation Types)
                </div>
              </div>
          ...
          <div class="process-phase" :class="{ 'active': currentPhase === 1, 'completed': currentPhase > 1 }">
            ...
            <div class="phase-detail">
              ...
              <div class="detail-section waiting-state" v-if="currentPhase < 1">
                <div class="waiting-hint">Waiting for ontology generationComplete...</div>
              </div>
...
const handleNewProject = async () => {
  ...
  try {
    ...
    error.value = 'project initialization failed: ' + (err.message || 'UnknownError')
  } finally {
    loading.value = false
  }
}
...
const loadProject = async () => {
  try {
    ...
    if (response.success) {
      ...
      error.value = response.error || 'load projectFailed'
    }
  } catch (err) {
    console.error('Load project error:', err)
    error.value = 'load projectFailed: ' + (err.message || 'UnknownError')
  } finally {
    loading.value = false
  }
}
...
const startBuildGraph = async () => {
  try {
    ...
    buildProgress.value = {
      progress: 0,
      message: 'Currently StartGraph Build...'
    }
    
    const response = await buildGraph({ project_id: currentProjectId.value })
    
    if (response.success) {
      buildProgress.value.message = 'Graph BuildtaskalreadyStart...'
    ...
    error.value = 'StartGraph BuildFailed: ' + (err.message || 'UnknownError')
    buildProgress.value = null
  }
}

// Graph DataPolling timers
let graphPollTimer = null

// StartGraph DataPolling
  ...
  try {
    ...
    if (response.success) {
      ...
      buildProgress.value = {
        ...
        message: task.message || 'Handle...'
      }
      
      console.log('Task status:', task.status, 'Progress:', task.progress)
      
      if (task.status === 'completed') {
        console.log('✅ Graph BuildComplete，LoadingcompleteData...')
        ...
        buildProgress.value = {
          progress: 100,
          message: 'Build Complete，Loadinggraph...'
        }
        ...
        if (projectResponse.success) {
          ...
          // Finalfull loadGraph Data
          if (projectResponse.data.graph_id) {
            console.log('📊 full loadgraph:', projectResponse.data.graph_id)
            await loadGraph(projectResponse.data.graph_id)
            console.log('✅ graph loadingComplete')
          }
        }
        ...
        stopPolling()
        stopGraphPolling()
        error.value = 'Graph BuildFailed: ' + (task.error || 'UnknownError')
        buildProgress.value = null
      }
    }
  } catch (err) {
...
const renderGraph = () => {
  ...
  if (nodesData.length === 0) {
    ...
    svg.append('text')
      ...
      .attr('fill', '#999')
      .text('WaitingGraph Data...')
    return
  }
  
  // createNodesmap for lookupName
  const nodeMap = {}
  nodesData.forEach(n => {
    nodeMap[n.uuid] = n
  })
  ...
  const nodes = nodesData.map(n => ({
    ...
    rawData: n // SaverawData
  }))
  
  // createNodesIDset for filtering valid edges
  ...
  // draw edges (contains clickable transparent wide line）
  ...
  simulation.on('tick', () => {
    // Updateall edge positions (including visible lines and transparent click areasregion）
